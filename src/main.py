...
print("Shapes are in order (N_sim, N_timesteps, N_dof) or (N, T, D)")
print(f"Encoder input shape: {x_train.shape}")
print(f"Decoder input shape: {y_train_shifted.shape}")
...
results = model.evaluate(x_test, y_test, batch_size=batch_size) # TODO increase batch_size?
print(results)
